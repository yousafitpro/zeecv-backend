        ...
        <tbody>
            @foreach($list as $item)
                <tr style="border-bottom: 0.02rem solid #e5e7eb;">
                    <td >
                        ...
                        <div class="m-list-item">
                            <div class="d-flex justify-content-between align-items-start mb-1">
                                ...
                                <a href="{{ route('pmm.products.update', product_encrypt($item->product->id)) . '?back_url=' . route('pmm.affiliate_links.view') }}"
                                   class="m-list-item-link">
                                    <img src="{{$item->product->attachment->file_url??''}}" style="width: 40px;border-radius:10px">
                                   {{ $item->product->name ?? 'No Title' }}</a>
                    ...
                    <td>
                        <br>
                        {{$item->product->price}}$ <span class="badge badge-info badge-sm" style="height:15px;width:30px;font-size:10px;padding:2px !important">{{$item->product->commission}}%</span>
                    </td>
                    <td>
