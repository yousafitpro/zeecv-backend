...
<div class="container-fluid">
    <div class="card shadow-sm">
        ...
        <div class="card-body">
              <a href="{{ $back_url ?? route('pmm.ledger.balance.view') }}" >
                <i class="fas fa-arrow-left"></i> Back
            </a>
            <br>
            <br>
           <form method="post" action="{{route('pmm.ledger.balance.updatePost',$item->id)}}" id="create_sprint_form" enctype="multipart/form-data" onsubmit="submitForm(event, this)">
            @csrf
...
<br>
        ...
        <div class="col-md-5">
           ...
           <select name="user_id" disabled class="select2" style="width: 100%" >
                <option value="credit">---Select---</option>
                @foreach ($users as $u)
                    <option value="{{$u->id}}" {{$u->id==$item->user_id?'selected':''}}>{{$u->email}} (#{{unique_encrypt($u->id)}})</option>
                @endforeach
      ...
      <div class="col-md-4">
            ...
            <input type="datetime-local" required name="created_at" id="created_at" value="{{ \Carbon\Carbon::parse($item->created_at)->format('Y-m-d\TH:i') }}" class="form-control">
        </div>
     ...
     <div class="row">
        <div class="col-md-12">
            ...
            <textarea style="height: 80px" disabled name="message"   class="form-control">{{$item->message}}</textarea>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <label for="zipCode" class="form-label" id="payment_iban">Internal Note</label>
            <textarea style="height: 120px" name="internal_note"  required class="form-control">{{$item->internal_note}}</textarea>
        </div>
    </div>
    <br>
