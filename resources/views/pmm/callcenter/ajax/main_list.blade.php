...
<a href="{{ route('Call.Service.Detail', unique_encrypt($item->id)) . '?back_url=' . urlencode(route('system.CallCenter.orders')) }}">
    <img src="{{ $item->link->product->attachment->file_url ?? '' }}" style="width: 40px;border-radius:10px">
    {{ $item->link->product->name ?? 'No Title' }}
        ...
        @endif
                    ...
                    <td>
                        ...
                        <div class="badge badge-sm" style="color: black">{{$item->link->product->payment_method}}</div>
                    </td>
                    <td>
                        @if (!empty($item->latestParcel) && !empty($item->latestParcel->user))
                            {{$item->latestParcel->user->name}} <br>
                            {{$item->latestParcel->user->email}} 
                        @endif
                     ...
                     <td>
                       ...
                       @if($item->order)
                         CC status :  {{$item->order->cc_status}}
                       @endif
