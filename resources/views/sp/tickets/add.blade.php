...
<style>
   ...
   <form method="post" action="{{route('sp.tickets.addPost')}}" id="create_sprint_form" enctype="multipart/form-data" onsubmit="submitForm(event, this)">
            @csrf
...
<input  type="text" name="subject" required class="form-control" id="fiscalCode">
        </div>
        <div class="col-md-2">
        ...
        <div class="col-md-4">
            <label for="legalEntity" class="form-label">Users (Default Support Team is pre-selected)</label>
            <select  name="users[]"  class="form-control select2" @if(is_admin()) multiple @endif>

          @foreach ($myusers as $id => $name)
                <option value="{{ $id }}">{{ $name }} (#{{unique_encrypt($id)}})</option>
            @endforeach
            </select>
        </div>
