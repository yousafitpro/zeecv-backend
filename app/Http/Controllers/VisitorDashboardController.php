...
class VisitorDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['sources'] = (clone $visitQuery)->whereNotNull('utm_source')
                        ...
                        ->groupBy('utm_source')
                        ->get();
        $data['top_clicked_jobs'] = (clone $visitQuery)
            ->whereNotNull('job_slug')
            ->select('job_slug', DB::raw('COUNT(*) as count'))
            ->groupBy('job_slug')          // group by the actual column
        ...
        $data['job_sources'] = (clone $jobQuery)->whereNotNull('source')
                        ...
                        ->groupBy('source')
                        ->get();
        $data['user_types'] = (clone $userQuery)
                        ->select('signup_type', DB::raw('count(*) as count'))
                        ->groupBy('signup_type')
                        ->get();
        ...
        $data['app_link_visits'] = (clone $visitQuery)
                ...
                ->count();
        $data['app_visits'] = (clone $visitQuery)
                ->where('request_type', 'mobile-app')
                ->count();
